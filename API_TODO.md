...
- [x] **Для achievements:** добавлен `/api/user-achievements`; старые `/api/achievements/user/:userId` и `/api/achievements/award` сохранены, добавлен `PUT /api/achievements/:id` в [`achievementsRoute`](server/src/api/routes/achievementsRoute.ts:14). `POST /api/user-achievements` принимает frontend-варианты `userId/achievementId`, `user_id/achivement_id` и вложенное `achievement.id` через [`UserAchievementsController`](server/src/api/controllers/UserAchievementsController.ts:33).
